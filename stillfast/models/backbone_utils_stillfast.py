...
from stillfast.models.noun_attn import Guided_Attn
import math
...
class StillFastBackbone(nn.Module):
    ...
    def __init__(
        ...
        if pre_pyramid_fusion:
            for layer, c2d, c3d in zip(layers, still_backbone_channels, fast_backbone_channels):
                if layer in cfg.GAIT_ATTN_LAYERS:
                    setattr(self, f"guided_attn_{layer}", Guided_Attn(num_frames=cfg.NUM_FRAMES, dims=c3d))
    ...
    def forward(self, x):
        ...
        if self.pre_pyramid_fusion:
            for layer in self.layers:
                ## fast_features[f"{layer-1}"] - [B, C, T, H, W]

                if layer in self.cfg.GAIT_ATTN_LAYERS:
                    fast_features[f"{layer-1}"] = getattr(self, f"guided_attn_{layer}") (fast_features[f"{layer-1}"], orvit_boxes)
