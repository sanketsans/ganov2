from stillfast.models.backbone import VideoSwinTransformerBackbone
...
class StillFast(FasterRCNN):
    ...
    def load_faster_rcnn_pretrained_weights(self, kwargs):
        ...
        # Load state dict
        state_dict = torch.hub.load_state_dict_from_url(kwargs['checkpoint_url'], progress=True)
    ...
    def from_config(cls, cfg):
        ...
        # backbone = VideoSwinTransformerBackbone(cfg)
    ...
    def forward(self, batch):
        images = [StillFastImageTensor(a, b,c) for a,b,c in zip(batch['still_img'], batch['fast_imgs'], batch['inter_boxes'])]
        targets = batch['targets'] if 'targets' in batch else None
        if targets:
            _ = [lst1.update(lst2) for lst1, lst2 in zip(batch['targets'], batch['extra_data'])]
        else:
            targets = batch['extra_data']
